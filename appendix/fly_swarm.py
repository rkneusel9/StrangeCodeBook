...
#  file:  fly_swarm.py
#
#  Evolve reduced-instruction Firefly programs to produce
#  desired displays.
#
#  RTK, 16-Jan-2021
...
#
def main():
    """Search for a Firefly program generating the desired display"""

    if (len(sys.argv) == 1):
        print()
        print("fly_swarm <target> <npart> <ndim> <niter>")
        print()
        print("    <target>   - 5x5 target display (.txt)")
        print("    <npart>    - swarm size")
        print("    <ndim>     - maximum program length")
    ...
    print()
    print("Firefly position (%d,%d)" % (x,y))
    print("(%d particles, %d/%d iterations, %d best updates, %d function evals, %0.3f sec)" % (npart, res["iterations"], niter, len(res["gbest"]), obj.fcount, en-st))
    print()
